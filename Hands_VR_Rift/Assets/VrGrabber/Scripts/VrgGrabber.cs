...
public class VrgGrabber : MonoBehaviour
{
    ...
    const float minGrabSmoothFilter = 0.15f;
		...
		public bool have_info_object = false;
        Ball origin_ball;
		public InfoObject info_object;
		public InfoObject io;
		public DeleteInfo delete_info;
		public DeleteInfo di;
		bool can_decre=false;
    ...
    void OnTriggerEnter(Collider collider)
    {
            ...
            {
                have_ball = true;
                origin_ball = collider.GetComponent<Ball>();
            }
            else if(collider.tag=="InfoObject"){
                have_info_object = true;
                info_object = collider.GetComponent<InfoObject>();
				if (info_object == null) {
					delete_info = collider.GetComponent<DeleteInfo> ();
				}
            }
        var grabbable =
    ...
    void OnTriggerExit(Collider collider)
    {
            ...
            {
				have_info_object = false;

				if (info_object != null) {
					io = info_object;
					info_object = null;
				} else if (delete_info != null) {
					di = delete_info;
					delete_info = null;
				}                
    ...
    void UpdateGrab()
    {
        ...
        {
            DirectGrab();
				if (have_info_object && info_object != null) {
					info_object.GrabInfoObject ();
				} else if (have_info_object && delete_info != null) {
					delete_info.GrabInfoObject ();
				}
                
            //RemoteGrab();　Rayで掴むところ
        }
        else if (isHoleEnd_)
        {
                if (have_ball && origin_ball != null)
                {
                    origin_ball.SwitchGravity();
                }
				...
				{
					can_decre = true;
					OnTriggerExit (info_object.GetComponent<SphereCollider> ());
					can_decre = false;
					io.ReleseInfoObject();
					io = null;
				}else if(have_info_object && delete_info != null)
				{
					can_decre = true;
					OnTriggerExit (delete_info.GetComponent<SphereCollider> ());
					can_decre = false;
					di.ReleseInfoObject();
					di = null;
				}
    ...
    void SecondGrab(VrgGrabbable grabbable)
    {
        ...
        primary.dualGrabInfo_.pos = grabbable.transform.position;
            primary.dualGrabInfo_.center = primaryPos;//(primaryPos + secondaryPos) / 2;
